...
Node.js reporter daemon. It collects local agent-usage and dev-stats, then POSTs
them to `/api/usage` on tkmx-server. Entry point is `reporter/report.ts`, run as
`npm run report` (`dist/reporter/report.js`); scheduled via launchd (macOS) or a
systemd timer (Linux). See `README.md` for setup, configuration and the full
data flow.
